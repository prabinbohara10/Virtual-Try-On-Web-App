...
def inference(net, data_root = "data/", img_path='', output_path='./', output_name='f', use_gpu=True):
    '''
    ...
    img = read_img(data_root + "test/image/"+ img_path)
    testloader_list = []
    ...
    parsing_im.save(data_root + "test/image-parse/"+'{}'.format(output_name).replace(".jpg", ".png"))
    #print("output img parse:", output_name)
    # gray image pasre
    cv2.imwrite(data_root + "test/image-parse-new/"+'{}'.format(output_name).replace(".jpg", ".png"), results[0, :, :])
...
if __name__ == '__main__':
    ...
    inference(net=net, data_root = opts.data_root, img_path=opts.img_path,output_path=opts.output_path , output_name=opts.output_name, use_gpu=use_gpu)
