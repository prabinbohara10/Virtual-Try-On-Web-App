...
protoFile = "/GMM/checkpoints/pose_deploy_linevec.prototxt"
weightsFile = "/GMM/checkpoints/pose_iter_440000.caffemodel"
nPoints = 18
...
def generate_pose_keypoints(img_file, base_root):
    '''
    Generates pose keypoints
    Input: Person Image
    Output: Writes json file with keypoints(shape: 18*3 = 54)
    '''
    img_path = base_root + "/GMM/data/test/image/"+ img_file
    print(img_path)
    ...
    t = time.time()
    net = cv2.dnn.readNetFromCaffe(base_root + protoFile, base_root+ weightsFile)
    #if args.device == "cpu":
    ...
    # write json file
    img_file = img_file.replace('.jpg', '')
    img_file = img_file.replace('.png', '')
    img_file = img_file.replace('.PNG', '')
    img_file = img_file.replace('.jpeg', '')

    json_file = img_file + "_keypoints.json" 
    with open(base_root + "/GMM/data/test/pose/"+ json_file, 'w') as outfile:
